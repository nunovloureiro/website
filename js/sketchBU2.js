...
var s = function(p) { // p could be any variable name
  ...
  p.draw = function() {
      ...
      let targetP11y = p.int(p.height/6+p.mouseY*0.01);

      let targetP2x = p.int(p.mouseX*0.7-p.mouseY/200);
      let targetP2y = p.int(p.height/4.3+p.mouseY*0.01);

      let targetP3x = p.int(p.mouseX*0.15+p.mouseY/2);
      let targetP3y = p.int(p.height/3+p.mouseX*0.01);

      let targetP4x = p.int(p.mouseX*0.5-p.mouseX*p.mouseY/2000);
      let targetP4y = p.int(p.height/2.3+p.mouseY*p.mouseX*0.01);

      let targetP51x = p.int(p.mouseX*0.66-0.3*p.mouseY);
      let targetP51y = p.int(p.height/1.3+p.mouseY*(-p.mouseX)*0.01);

      let targetP6x = p.int(p.mouseX*2-1.3*p.mouseY);
      let targetP6y = p.int(p.height/1.1+p.mouseY*(-p.mouseX)*0.1);
